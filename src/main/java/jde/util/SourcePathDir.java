...
/**
 * A SourcePathEntry that represents a directory in which source files are
 * stored.  This is scanned recursively for classes at load time.
 *
 * Copyright (C) 2019 Troy Daniels (udalrich.schermer@gmail.com)
 *
 ...
 *
 * Created: Wed Feb 6, 2019
 *
 * @author Troy Daniels
 * @version $Id: SourcePathDir.java,v 1.4 2003/10/20 03:55:49 paulk Exp $
 */

class SourcePathDir extends ClassPathEntry {
    private File directory;

    /**
     * Create an instance of SourcePathDir representing
     * <code>directory</code>
     *
     * @param directory a <code>File</code> value
     * @return a <code>SourcePathDir</code> value
     */
    SourcePathDir (File directory) {
        super();
    ...
    void addRecursively(File directory, File rootDir) {
        ...
        for (int i = 0; i < files.length; i++) {
            current = files[i];
            if (current.toLowerCase().endsWith(".java")) {
